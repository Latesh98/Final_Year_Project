...
public class knn {
	...
	public static void knn(String trainingFile, String testFile, int K, int metricType){
		...
		try {
			...
			for(int i = 0; i < numOfTestingRecord; i ++){
				TrainRecord[] neighbors = findKNearestNeighbors(trainingSet, testingSet[i], K, (metal) metric);
	...
	static TrainRecord[] findKNearestNeighbors(TrainRecord[] trainingSet, TestRecord testRecord,int K, metal metric){
